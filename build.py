...
# -*- coding: utf-8 -*-
"""
JRD LinkedIn dashboard — datová vrstva.

Najde NEJNOVĚJŠÍ soubor 'JRD ContentPlan*.xlsx' ve složce o úroveň výš
...
def find_workbook():
    ...
    for d in SEARCH_DIRS:
        cands += glob.glob(os.path.join(d, "JRD ContentPlan*.xlsx"))
    # vyřaď dočasné lock soubory (~$...)
    cands = [c for c in cands if not os.path.basename(c).startswith("~$")]
    if not cands:
        sys.exit("Nenalezen žádný soubor 'JRD ContentPlan*.xlsx'.")
...
def parse_posts(wb):
    ws = wb["📃Content plan "]
    hr = find_header_row(ws, ["Name of profile", "Status", "N. IMPRESSIONS", "Date of posting"])
    if hr is None:
    ...
    for r in range(hr + 1, ws.max_row + 1):
        ...
        posts.append({
            ...
            "comments": int(num(g(r, "N. COMENTS")) or 0),
...
def parse_network(wb):
    ...
    for r in range(hr + 1, ws.max_row + 1):
        name = g(r, "Name")
        if name not in PERSONS:        # vyřaď #REF!, Ambassador 3-7, prázdné
            continue
        d = g(r, "Date")
...
def build(posts, status_counter, total_prepared, net):
    ...
    persons_in_posts = ["all"] + [pp for pp in ["Jan Řežáb", "Jan Sadil"]
                                  if any(p["name"] == pp for p in posts)]
    ...
    for yr in years:
        stacks = {}
        for person in ["Jan Řežáb", "Jan Sadil"]:
            arr = [0] * 12
            for p in posts:
    ...
    for yr in years:
        yearly[str(yr)] = {
            "Jan Řežáb": sum(p["imp"] for p in posts if p["date"].year == yr and p["name"] == "Jan Řežáb"),
            "Jan Sadil": sum(p["imp"] for p in posts if p["date"].year == yr and p["name"] == "Jan Sadil"),
        }
